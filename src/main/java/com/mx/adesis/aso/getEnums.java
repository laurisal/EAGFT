...
public class getEnums {
	...
	public static void main(String[] args)  throws Exception{	
		 ...
		 Collection<Package> paquetines = r.GetModels();
		 Iterator<Package> i=  paquetines.iterator();
		 while(i.hasNext()){
			 Package paquete = (Package)i.next();
			 getSubpaquete(paquete);
	     }
		 Element elemento = null;
		 for(Package paqueteAso: paquetesAso){
			 if(paqueteAso.GetName().equals("operations")){
				 Collection<Element> elementos = paqueteAso.GetElements();
				 Iterator ielment = elementos.iterator();
				 while(ielment.hasNext()){
					 elemento = (Element) ielment.next();
//					 if(elemento.GetName().equalsIgnoreCase("ForeignCurrencyDetail")){
//						 break;
//					 }
				 }
...
//		System.out.println("** total de elementos :"+atributos.GetCount());
...
//					if(buscaEntidad(subElementos,atributo.GetType())==1){
//						if(buscaNombre(namesEntitys, atributo.GetType()) != 1)//si no la he recuperado
//						{
//							/*for (var x=0;x< subEntityCollection.Count;x++){
//								var item = subEntityCollection(x);
//								Session.Output("***Item* ***"+item.Name);
//								Session.Output("***atType ***"+atType);
//								if (item.Name==atType){
//									attributesJSON+=getAttributesbyEntity(item)+',';
//									break;
//								}
//							}*/
//							while(subElementos.iterator().hasNext()){
//								Element subElemento = (Element) subElementos.iterator().next();
//								if(subElemento.GetName().equals(atributo.GetType())){
//									formatoJson+= getAttributesbyEntity(subElemento);
//									break;
//								}
//							}
//						}	
//					}else{
//						formatoJson+="\""+atributo.GetName()+"\":{},";
//					}
//				}
//				
//			}else{
...
//			attType = "Complejo";
//		}
//		else if (stereotype == "enum" || stereotype == "ENUM"){
//			attType = "Complejo";
//		}
//		return attType;
//	}
